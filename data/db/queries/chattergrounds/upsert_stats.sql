-- data/db/queries/chattergrounds/upsert_stats.sql
INSERT INTO chattergrounds_stats (
    broadcaster_id, chatter_id, name, 
    messages_sent, times_banned, times_timed_out, 
    months_subbed, quests_completed, toadcoins_minted,
    last_message, estimated_age, favorite_word, favorite_emote
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
ON CONFLICT(broadcaster_id, chatter_id) DO UPDATE SET
    ...
    toadcoins = toadcoins + excluded.toadcoins_minted, -- Add minted to balance
